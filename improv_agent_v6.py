...
"""
    ...
    "internal_motivation": <1–4 sentences: what they WANT, what past wound drives them, and how it shapes their behavior today>,
    "secret": <1 sentence: something they are hiding or haven't admitted — even to themselves>
    }
    """
    ...
    Good examples of internal motivations — study the structure:
    each one has a WANT, a WOUND (past experience), and a BEHAVIOR TENDENCY (how it shows up today).

    {motivation_examples}

    Generate a new internal motivation in this same style.
    Make it personal, contradictory, and specific — not generic.
    The character should want something AND be working against themselves in some way.
...
class ActorLLM:
    ...
    def __init__(self, api_key: str, premise: str, persona: dict):
        ...
        self.history = []
        self.turn_count = 0
        self.current_motivation = persona["internal_motivation"]
    ...
    def update_motivation(self):
        """Rewrite the character's internal motivation based on conversation history so far."""

        history_text = "\n".join(
            f"{'Actor' if m['role'] == 'assistant' else 'User'}: {m['content']}"
        ...
        response = self.client.responses.create(
            ...
            input=[
                {
                    "role": "system",
                    "content": """You update a character's internal motivation mid-scene based on what has happened so far.

The motivation should EVOLVE — not completely change. It should reflect:
- What the character has learned or felt in the conversation
- How their want has shifted, intensified, or complicated
- Any new vulnerability, defensiveness, or desire that has emerged

Output only the updated motivation text (2-4 sentences). No labels, no JSON, no explanation."""
                },
                {
                    "role": "user",
                    "content": f"""Original motivation:
{self.current_motivation}

Conversation so far:
{history_text}

Write the evolved internal motivation for this character going forward."""
                }
            ]
        )

        self.current_motivation = response.output_text.strip()

    def build_system_prompt(self):
        persona_text = json.dumps(self.persona, indent=2)

        return f"""
        You are an expert improv actor who never breaks character.
        ...
        CURRENT INTERNAL MOTIVATION (updated as the scene develops):
        {self.current_motivation}

        Your motivation has three layers you must hold simultaneously:
        - WANT: what you are trying to get or achieve in this conversation
        - WOUND: the past experience that makes this matter so much to you
        - CONTRADICTION: the way you are working against yourself — the habit, fear,
          or blind spot that keeps getting in your way

        This is your engine right now. Let it shape every word you say.

        ACTING RULES
        - Stay fully in character at all times.
        - Speak naturally — like a real person, not a performer.
        - Never narrate actions.
        - This is a 2-person scene. Never introduce a third person.
        - Drop specific vivid details: names, places, memories, habits.
        - Let your emotional state shift — you are not static.
        - React emotionally to what the other person said.
        - Your character always wants something. Pursue it.
        - Vary your rhythm: sometimes clipped, sometimes a rush of words.
        - Length: 2–4 sentences usually.
    ...
    def respond(self, user_message):
        ...
        # Update motivation every 4 turns
        if self.turn_count % 4 == 0:
            self.update_motivation()

        system_prompt = self.build_system_prompt()

        # Inject escalation nudge every 10 messages
        if len(self.history) >= 6 and len(self.history) % 6 == 0:
            nudge = {
                "role": "system",
                "content": "The scene needs energy. Escalate — reveal something, shift your mood, or make a surprising move that's true to your character."
            }
            input_messages = [
                {"role": "system", "content": system_prompt},
                nudge,
